...
describe("middleware auth redirects", () => {
  it("redirects unauthenticated user from /ideas to /login", async () => {
    ...
    const res = await middleware(request("http://localhost/ideas"));
  ...
  it("redirects unauthenticated user from /ideas/abc to /login", async () => {
    getUserMock.mockResolvedValue({ data: { user: null } });
    const res = await middleware(request("http://localhost/ideas/abc"));
    expect(res.headers.get("location")).toBe("http://localhost/login");
  });
  ...
  it("does not redirect unauthenticated user from non-/ideas paths", async () => {
    getUserMock.mockResolvedValue({ data: { user: null } });
    const res = await middleware(request("http://localhost/login"));
    expect(res.headers.get("location")).toBeNull();
  });
});
